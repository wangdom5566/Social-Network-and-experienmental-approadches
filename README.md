- 課程介紹(老師談)
  - 本週網絡、下週社會心理學、網絡無所不再，但網絡通常是直接蒐集網絡資料然後來分析。
  - RDS這個方式，在於，知道網絡很重要，這群人也很重要，但作為"hidden population"不容易接觸到，和犯罪、標籤化有關的次群體有關。
  - RDS最初能追到公共衛生和芝加哥大學的都市研究？
  - RDS強調incentive、給受試者要的東西、coupon當證據？可以知道誰轉介。
  - 數學證明上說明這個是Markov chain，隨機過程，a狀態到B狀態是個轉折過程，最後收斂批評和原先樣本無關。
  - Markov Chain轉移矩陣
  - hidden population不知道母體，只能透過相關的方法抽。
  - 但要多少波才能收斂？
  - 歸類？經驗法則？敏感度？值閾可以在研究者經驗上處理出來？
  - 實驗的政策性導向？怎麼減少吸毒、販毒、針頭使用？透過實驗方式，有人政策受益，有人則否，實驗對照來看成果。
  - 第二篇heckathorn有七八種方法的調整，網絡大小，跟多少人接觸叫做degree。
  - 模擬，先假設分布，再選方法重抽，來看哪個模擬跟現實抽樣比較像？
- 同學介紹討論文章
  - 抽樣
  - 討論第二篇經濟學實驗設計
  - 如何用大學宿舍當實驗基地
  - 共線性問題、內生性問題？實驗設計怎麼處理treatment的這兩個問題？
    - 實驗設計會處理這個問題
  - 因果模型
  - missing value的處理？是不是要把不回答的東西放進去？
    - 社會網絡的missing value少掉了人，還少掉關係，一去掉可能就去掉很多。跟一般抽樣調查的missing不一樣。
    - social discounting？temporal discounting？
    
## 開放討論，重新驗證結果
## 第二週：談合作-一篇社會學和一篇生物學博奕學
- 第二週
  - 合作是個老議題，也是持續發展的新議題？
  - 合作是science裡面25個未來的大問題，中，認為重要的，接近社會科學、人文的主題。
  - 合作是個生物學重要的老議題？
  - 合作是犧牲自己的利益，換取公眾的利益？
  - rational choice？研究很多東西根本是不理性的行為？沒有道理大家才研究它，社會學喜歡把經濟學歸類為Rational？所以經濟學有想要改名？
  - 50 60 年代有出來一個社會交換理論，goffaman的東西也稍微有點獎，這一派在80-90年代在社會學曾經主流，但現在式微了
  - 這個議題在社會學不算新，題材起起落落，生物學和政治學似乎有把他接起來。
  - Peter Kollock是華盛頓大學社會交換理論的重要學者。
  - Peter Kollock的social dilemma回顧文章，談這個社會困境非常經典。
    - 重要議題：囚犯兩難的奈許均衡，最後都不合作是最佳解
    - 最後被困在那邊，Social trap
    - 談社會機制，怎麼到social optimal？
    - Peter Kollock的tit for tat
    - Robert Extra用Repeated Game的方式，則這樣的博奕會變化嗎？(TIT FOR Tat策略最好？)
    - 對經濟學家、政治學家、生物學家來說，最想要的是知道最佳解，也就是哪個會存活？
    - 依照對方上一次行動決定這次的行動，但第一次的決定怎麼做？
    - 但Peter Kollock認為Robert的文章不完全隊，所以提出一個反Robert Extra
    - Peter Kollock之後政治學又有一篇批判，登在ASR的回章，然後Kollock又回應。Johnathan Bender
    - network reciprocity社會學意味的合作、群與群的競爭(gruoup selection，政治學的合作)
    - network reciprocity的實驗？把原來的東西用新的策略取代，生物學叫selection，政治學或經濟學叫learning mechanism 
    - 用實驗法找不同階級？不同社會標籤的東西的東西？
    - 把這個Game或實驗帶到真實世界，然後保持一些假說？來做檢視？用比較對照的檢視工具和理論。
    - Johnathan Bender對kollock的批判
       - 1.東西不是新的
       - 2.In general,simulation results cannot prove that astrategy is evolutionarily stable----with or without noise.The essence of a 
           stable strategy is that it cannot be invaded by any mutant strategy,and this can only be established.
       - 3.noise解決惡性循環？突然把Tit for tat的問題跳出。
       - 4.對抗全球暖化，兩個社會或多國之間如何合作解決氣候變遷？減碳？
 
 ## 第三周：結構洞
 
- 弱連結和強連結：strong and weak ties，雖然weak ties弱，雖然訊息傳不快，但有更多元的資訊流通。
- 兩個網絡間不相連，謂之結構洞，spanning the structure hole
- 組織社會學？經濟社會學？
- 結構洞位置愈好、constrain值愈低，則人緣和薪水都比較好。
- 經濟學96年開始把均衡的東西帶進社會網絡
- 網絡穩定的時候，沒有一個人可以，建新的一條線或減少一條線，來不損害別人的利益？
- 列出了幾個穩定網絡
- vincent的文章可以當作期末操作指南。
- 組織社會學，結構洞？組織的網絡。
- community detection，分群與社會網絡？

##  第五周：中心性
- 社會交換理論，認為依賴度的是權力來源，A對B的依賴越高，則B對A的權力就越大
- 有了結構產生依賴性，有了依賴性就產生了權力，此即社會交換理論
- 看看中心度和power依賴性是不是有關，degree是有多少連結，理論上連結的多，可能中心度高。
- closeness centrality步數，看和所有點的距離，理論上中心度愈高，到所有點總和愈短。
- A到B點有多少種走法，有多少走法要經過某點，即該點的中心度，不只衡量距離，也衡量多少步驟要經過誰。
- 當網路不是談流通概念，而是談競爭與交換概念，則這些指標能否適用？
- 如約會、找工作、
- 作為中間仲介者，如果交換的雙方更強勢，彼此互賴的是交換雙方。則中間仲介者權力沒那大。
- 交換理論看哪個點敢開得比較高價碼，另一方能不能接受。
- eigenvalue解bonarchi的關係矩陣R
- betweeness和structure hole的連結度相當高，但主要還是正向關係，但除了正向關係之外還有一些負向關係在？
- 每個中心度的目標和意涵不一樣，而Eigen vector則也是有雙向的。
## 第六周：公平感和文化
- Joseph Henrich，是一個人類學家，但擅長用經濟學的模組方式和工具來做實證研究，也就是今天談得關於實驗的東西。
- 田野研究，至少蹲一年，Henrich當初挑祕魯部落研究，還有大洋洲部落。UCLA有少數做實證和Model的人類學家。
- 經濟學願意接受她，是因為該教授帶出新的研究視野，做出了一個研究對人類學或經濟學都有貢獻。
- foundation of human society，動員超過20-30位的學者寫這本書，做人類學和經濟學交錯的研究，
- 行為經濟學和實驗經濟學興起後，挑戰賽局的說法。
- ultimate game，P1-P2，P1決定分法，P2決定要不要接受，不要接受的話雙方都拿0，如果對分就是5050，稱為fair的結果。
- 當一般的小錢時，大致上均衡點在64或73對分，而即使將相當的工資拿到比較窮國去分，分的比例儘管下降，但結果大致如此。
- fairness的現代文明，大概就是對分到30%
- Henrich則提到原始社會的公平感、手足和連帶，Henrich做的實驗意義在，原是部落不見得公平和在乎正義感？
- 演化心理學，90-2000年的時候，認為遺傳影響現代一個行為，一些原始的遺留。
## 第七周：網絡與傳播
- centola。重複的訊息和反複核查才是弱連帶理論窯論Granovatter，complex contagion和structure。
- segregation model和agent base model的風潮，引起風潮。
- 和早期的社會學家，還有很多做擴散理論、傳播學界、公衛學界對話。
- coleman做醫療社會學研究出的credibility？
- legitimacy和credibility很像？人多一點會有大一點的正當性。另外還有emotion contagion的傳染？
- 一個人不夠，三四個人感染。光談simple和complex就可以花上一頁。
- long ties和short ties的差別。
- 橋梁比較多在Random少在真實世界？random type會比較多。
- reduntant的多餘姓？門檻、還有多餘影響？把東西傳出去。
- p=0是lattice，很局部的狀況，也許會有很多的門檻需要克服。
- p=1是random的結構，傳染或公共衛生模型的門檻。
- 
## 第八周：社會不平等
- Messie Salgnik,Duncan Watts發在Science的文章，不平等的概念，做量化特別會研究這個部分，一世紀？從Micro到Macro的層次，
- James stoneman，Macro到Macro和，Macro到Micro(社會心理學)，另外是Micro到Macro的層次，纏腳的習俗接受不接受？
- 今天兩篇文章主要處理Micro到Macro的問題。把代際因素放進來，除了天分外，還有合作策略
- 天分與否，到收入上，很直接的關係，但本文不認為，社會影響，眾人受影響的共犯，這周傳達的東西很簡單，就是透過實驗檢視，
- 社會影響造成了評分的落差，網路上的實驗？
- 大數據社會學？時髦，這期的台灣社會學，bit by bit ，洪晨碩？本期的台灣社會學，撈了資料作分析？分析社群媒體與政府資料，
- 抓了不同的資料來分析。但本文是2006就開始做的，收了14000多份樣本。
- 東亞研究中心比較傳統的抽樣調查方式
- 上一周demon centola，他為什麼很快收數據上來，重點是兩個字，是趣味。然後就能收比較多的樣本。
- 希望實驗有點趣味可以吸引人，上星期或這星期或都有一點意思，所以能吸引人來。
- 實驗設計相當成功，三個組別，實驗一二的差異是是否有排序、下載量。
- 結果表面好的被捧更好，壞得被捧更壞，炒作了一波造成不平等擴大。
- 人氣支持或比較差不多，有對照能看出這個不平等的效應多大。
- 前衛的概念、論文實驗設計也是需要資源。
- 不平等的其他類型

- 不平等的合作遊戲
- 自私自利的情況下，到合作？怎麼做的？
- 把合作拉到不平等問題，在實驗文獻裡面，一般人的財富不均等，決定要不要放在中間的籃子裡，會增加，
- 則到底要拿出多少？目前對應的最大的案例是全球暖化問題？
- 從微觀到巨觀，怎麼處理？
- 過程，機制的處理？
- 參與觀察也可能有限，人氣堆疊透過實驗來觀察當中過程？
- Noah Friedkin
- 三個人都有意見，對於政治人物的支持有差異度
- 三個人事部是有接觸
- 對你的影響力有多高？彼此的影響力有多高，怎麼互相影響。
- 數理社會學家，線性代數的系統，輿論和社會網絡的影響力怎麼互相變化？
- 目前出現的收斂很常往兩極化的狀況。
- 真實世界上可能要反過來推，測一下兩者之間的影響力，指標怎麼做。
- 股票的狀況？集體的行為。
- 影響的網絡https://scholar.google.com.tw/citations?user=uPCKsFEAAAAJ&hl=zh-TW

## 第十周

- 這篇文章還有電線有關的資料庫和腦神經有關的資料庫。只有一篇和人有關。
- 小世界回顧討論六度分隔，指出90幾個案例中，只有18個人完成，驗證六度分隔理論？
- 只有中位數大概是六度。
- Duncan Watts第二篇論文用email複製過去六度分隔實驗，來驗證之前的理論。
- 實驗設計有趣的話，也許可以省下一些費用。
- 有九萬多人加入實驗，當中25%人提供個資，最後有兩萬四千多個chain
- 找了18個人物當作目標，要成功寄信給他
- 但Duncan Watts發現，儘管電郵的path甚至更少，但是有很多訊息傳不出去，30%成功而已。
- 算進退潮率才能勉強提高。
- Duncan Watts的本文提到incentive的重要性。小世界的遊戲？肉搜？
- Milgram的時代只做美國中西部到紐約的驗證，但Duncan Watts可以做到全球，發現步數距離
- 也大概六步到七步。
- Centola認為距離短不代表什麼，社會心理學家有做一個研究，處理social distance discounting。
- 利潤、情感大概怎麼和distance互動？
- centola指出人要集眾，要很多周邊的人影響，才能互相影響，即使3.5步相隔，也沒有太大的影響。
- 叢聚性代表向心力，社會學很在乎這個，距離部署代表散播性，也許公衛學者會更在乎。
- 第一篇有出在AJS，甚至後來有出書，而Nature只有最有代表性的beta model。
- moral machine experienment
- ai在緊急狀況時，可能會撞到人，該怎麼辦？
- 林南認為人跟人有位階，不一定連到會理你，所以聯繫到互助有不一定的意義。
- 位階到社會資源、交換理論。
- 11年，美國的國防部做找氣球的實驗，然後丟一張氣球照片，看看大家能不能找到。
- 第一名由MIT拿下，花九個小時把十個氣球報上。
- 肉搜實驗當期末報告？

## 第十一周
- Coleman
- social order is not simple aggregation.
- Macro→      Macro
-  Micro→Micro
- 從個體到總體，就是社會選擇理論，如何討論不同Micro屬性的總和，變成總體，
- Behavioral experiments on biased voting in networks是共識和個體如何取決。
- 能不能在日常生活中舉出這樣的例子。
- 一致決上，成果可能比較完整，不會被扯後腿影響。
- 除了報酬的不對稱性，網絡也是一個重要的變因。
- ER Random network 完全沒規則的連線。
- PA preference assession model？
- co cohesion的網絡，
- 一堆偏好下，必須妥協，然後有兩機制可以把妥協達成，需要跨次群體、同溫層的連結才能妥協，另外還需要誘因機制。
- Minority什麼時候可以扭轉局勢，tastely stubborn？
- 江這篇處理合作的東西，主要也是從Micro到Macro。經濟學家在80年代玩Public goods game時，人的規模愈大，大家
- 愈自私愈保守，時間愈長，人也會愈自私保守。
- 如何處理大群體的公共選擇。
- 江彥生文章重點在選舉制度，當時在雨傘運動時，
- 跑去烏干達做研究，找一個出來幫一群人做抉擇。
- 要麼用選要麼用抽籤，選得方式合作程度愈高。
- 合法性？
- 比較直選和間接選舉的效果？
- 合作程度隨機指派0-1之間。給每一個領導人分數。
- 對於合作程度的偏好每個人不同。
- 在這種合作偏好狀況下，直選的偏好選擇容易合作的。間接選舉比較偏好選擇不合作？
- 間接選擇時比較容易選自己？
- 直選和間接選舉可能造成制度上的差別，直選時願意選擇合作的人當她的頭？
- 政黨普涵化？
- 第一篇文章認為跨越次群體同溫層的溝通，以及試當的固執。
- 第二篇文章中，認為合作和直選、間接選舉可能有影響。
- 從Micro到Macro，尤其是統計模型，似乎機制沒辦法很好的抽絲剝繭出來，實驗與模擬可以補足量化上這樣的不足？
- Social mechanism的學派？social analysis？
- 續任問題的解決？江這篇沒有做。
- 受試者是台大學生為主。
- 平均每個人最後拿到多少報酬？六百多塊。
- 小團體，參與式預算和民主整合？
## 第十二周
- 演化賽局？為什麼要這麼做要在江2008解釋。然後模擬完在2010決定找人來實驗。
- 引用2010的模型最多反而是生物學家，biological market，動物界裡面，求偶或尋找合作對象是個market？
- social exchange theory獨到的時候，給你一個結構，決定你當中的權力與交換。
- 當初修綜合學門的課，尤其是賽局，江指出結構不是固定的，然後從課堂報告發展成論文。
- 社會學家比較少向經濟學家或心理學家，很少問結構怎麼來的？
- 對話對象不完全是社會學，第二篇比較像是對話生物學家。
- Martin Novak，演化賽局和一般賽局不太一樣，在乎賽局的動態和多元性，不同的策略和species彼此競爭。
- 當中的模型缺乏一個概念，當個體與個體互動時，是一個隨機的概念，草原上有一群生物，有一群很在乎公平、有一群很自私。
- 隨機相遇的種群，最後誰會留下來？吃虧就是占便宜。
- 老師把network不固定，變成選擇結果。
- 自發性地選擇概念放進去。
- 你怎麼選我，我怎麼選你，把這東西架設在一個基礎上，pay off的東西。
- attraction的機制在於利益？
- 實驗室多人對多人，要找場地和電腦和受試者、程式。
- 要寫一個程式十台電腦可以溝通，然後程式可以幫你做一個事情。
- 使用gale-shapley matching algorithm
- 戶外的配對節目？
- stability？平衡、穩定的配對？
- table2問了一個問題，誰會和誰配再一起。偏好是如何重要？
- 多人互動的實驗需要好的程式平台？
- 真實的實驗數據上再做模擬看看什麼時候收斂？
- Barclay & Willer在史丹佛社會學，仍然在做一些合作遊戲和公平遊戲。
- 實驗資料抽出來，當作參數，放入agent based model看看會不會收斂。
- 有個改變偏好的模型。最後也有類似的結果。
- 策略多的話，可能要模擬的次數非常多。
- 數學家和物理學家，也認為模擬沒有保證到底多少次才會收斂。
- 下星期換成和AI有關的題目。針對西班牙公投選舉做得預測。
- 期末報告
- 10號最後一次上課。隔一周交一個，交代一個實驗設計，還有想法，五頁到十頁的東西。

## 第十三周 AI與網路？

- ai怎麼作道德判斷，寫在腳本裡的道德困境。利他行為、合作、文化價值？
- ai與人類社會的價值，如何規範。
- The Rise of Social Bots，如何偵測？機器人和機器人互聯的比率比較高？
- 網路的community detection，猜哪一群可能是機器人，機器人互相連結度可能比人類高很多。
- 把確定是真人的資料找出來，把他們的屬性歸類，和機器人比對？
- 




      
    
  
